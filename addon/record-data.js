...
export default class M3RecordData {
  ...
  /**
   * We need to go through every single record and sync their states, e.g _inFlightAttributes, _data
   * because we have no idea if there are any local changes deeper in the model that is not part of
   * the server response until we go through all of them.
   */
  _syncNestedModelUpdates(attributes) {
    ...
    childRecordDatas.forEach(childRecordData => {
      if (!Array.isArray(childRecordData.data)) {
        ...
        // Optimization to stop unnecessary recursive calls.
        // If the server payload contains changes to an array,
        // since arrays are destroyed and recreated, we don't need to
        // recursively sync the nested model under this array.
